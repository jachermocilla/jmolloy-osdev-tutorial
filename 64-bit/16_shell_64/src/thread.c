...
// The same walk, keyed on process id. It returns the first thread it finds with
// that pid, which is unambiguous here only because a forked process has exactly
// one thread. A multi-threaded process would need this to say *which* thread,
// and wait() to mean something more careful than "the process finished".
static thread_t *find_by_pid(u32int pid)
{
    for (thread_t *t = all_threads; t; t = t->all_next)
        if (t->pid == pid)
            return t;
...
static thread_t *thread_alloc(u64int rip, u64int arg, u64int cs, u64int ss)
{
    ...
    t->pid        = t->proc->pid;   // Same process as our creator.
...
// wait(). Read this beside thread_join() above: line for line it is the same
// routine, and the only edits are that it finds its target by pid instead of
// tid and it drops join's "is this thread joinable" guards, because a child is
// its parent's to reap by definition.
//
// The child may already be a zombie when we ask -- it exec'd something short,
// which ran and exited before we got the CPU back. Then there is nothing to
// wait for: we skip the block and go straight to collecting the corpse. Or the
// child is still running, and we hang ourselves on its `joiner` hook and block;
// its own thread_exit() will unblock us on the way out. Either path ends at the
// same three lines, freeing the kernel stack and the thread_t that thread_exit()
// deliberately left behind for us. The address space is already gone -- the
// child's last thread freed it in thread_exit() -- so a zombie is cheap: a
// thread_t, a kernel stack, an exit value, and its pid.
int proc_wait(u32int pid, u64int *status)
{
    u64int f = irq_save();

    thread_t *t = find_by_pid(pid);
    ...
    if (t->state != THREAD_ZOMBIE)
    {
        t->joiner = (thread_t *)current_thread;
        block();
        // We wake here with interrupts still masked, exactly as join() does.
    }
