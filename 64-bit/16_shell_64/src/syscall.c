...
static u64int sys_getchar(void)          { return (u64int)(u8int)keyboard_getchar(); }
...
static registers_t *syscall_handler(registers_t *regs)
{
    ...
    if (regs->rax == SYS_EXEC)
    {
        // exec needs the frame, not just an argument: on success it rewrites it
        // to fall into the new program, and the value in rax is only ever seen
        // by the caller if exec *failed* and there was no new program to run.
        regs->rax = (u64int)do_exec(regs, (const char *)regs->rdi);
        return regs;
    }

    if (regs->rax == SYS_WAIT)
    {
        // wait() does not need the frame the way fork/exec/exit do -- it only
        // blocks and returns a value. It sits here beside them anyway rather
        // than in the syscalls[] table, because the table is a dense array
        // indexed 0..13 and SYS_EXEC already stepped outside it. Keeping the two
        // process-lifecycle calls together reads better than threading a
        // fifteenth hole through the table.
        u64int status = 0;
        int r = proc_wait((u32int)regs->rdi, &status);
        regs->rax = (r < 0) ? (u64int)-1 : status;
        return regs;
    }
