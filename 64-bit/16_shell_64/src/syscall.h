...
// Wait for the child process `pid` to finish, and return its exit value (or -1
// if there is no such child). Blocks in the kernel until the child exits, the
// same way syscall_getchar() blocks until a key is typed: ring 3 sees a call
// that takes as long as the child does to run. This is the primitive chapter 15
// did without, and the only thing standing between fork+exec and a shell.
DEFN_SYSCALL1(wait,              SYS_WAIT,              u64int)
