...
void switch_page_directory(page_table_t *pml4)
{
    ...
    current_pml4_phys = (u64int)pml4;   // placement-allocated: virt == phys
...
// ---------------------------------------------------------------------------
// Address-space cloning (fork).
//
// A page table for an address space must be reachable by its *physical*
// address, because that is what CR3 and the frame pointers in every table entry
// hold. The direct map from chapter 7 makes that reachability free: physical
// address P is mapped at virtual address P, so a raw frame doubles as a
// writable table we can fill in. Every table below is allocated this way, which
// is why we never touch kmalloc here -- kmalloc lives in the higher half, where
// virtual and physical diverge, and CR3 would get the wrong number.
// ---------------------------------------------------------------------------

// Grab a fresh frame from the bitset and return its physical address. Because
// of the direct map, that address is also where we may read and write it.
static u64int alloc_phys_frame(void)
{
    u64int idx = first_frame();
    if (idx == (u64int)-1)
        PANIC("No free frames (address space clone)!");
    set_frame(idx * 0x1000);
    return idx * 0x1000;
}

static u64int alloc_zeroed_table(void)
{
    u64int phys = alloc_phys_frame();
    memset((u8int *)phys, 0, 0x1000);   // phys == virt in the direct map
    return phys;
}

// Copy one page-table level of the lower (user) half. `level` counts down:
// 4 = PML4 slice, 3 = PDPT, 2 = PD, 1 = PT (leaves).
static u64int clone_table(page_table_t *src, int level)
{
    u64int dst_phys = alloc_zeroed_table();
    page_table_t *dst = (page_table_t *)dst_phys;

    for (int i = 0; i < 512; i++)
    {
        page_t *se = &src->entries[i];
        if (!se->present)
            continue;

        if (level == 1)
        {
            // Leaf. This is where the share-kernel / copy-user rule lives.
            if (se->user)
            {
                // A user page: give the child its own private copy.
                u64int frame = alloc_phys_frame();
                memcpy((u8int *)frame, (u8int *)((u64int)se->frame << 12), 0x1000);
                dst->entries[i] = *se;
                dst->entries[i].frame = frame >> 12;
            }
            else
            {
                // A supervisor page (the kernel's own low-half mappings, e.g.
                // the identity map): share the frame by reference.
                dst->entries[i] = *se;
            }
        }
        else
        {
            // Interior node: recurse, and point at the fresh sub-table.
            page_table_t *child = (page_table_t *)((u64int)se->frame << 12);
            u64int sub = clone_table(child, level - 1);
            dst->entries[i] = *se;
            dst->entries[i].frame = sub >> 12;
        }
    }
    return dst_phys;
}
...
{
    page_table_t *src = (page_table_t *)src_pml4_phys;
    u64int dst_phys = alloc_zeroed_table();
    page_table_t *dst = (page_table_t *)dst_phys;
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        if (i >= 256)
        {
            // Upper canonical half -- kernel only. Share the entire sub-tree by
            // reference, so any kernel mapping made after the fork (a new heap
            // page, a new kernel stack) appears in the child automatically.
            // This is the invariant chapter 9 leaned on: the kernel is
            // identical in every address space.
            dst->entries[i] = src->entries[i];
        }
        else
        {
            // Lower half -- may hold user pages. Walk it.
            page_table_t *pdpt = (page_table_t *)((u64int)src->entries[i].frame << 12);
            u64int sub = clone_table(pdpt, 3);
            dst->entries[i] = src->entries[i];
            dst->entries[i].frame = sub >> 12;
        }
    }
...
// Like clone_table, but for building a kernel-only address space: share
// supervisor frames by reference and skip user leaves entirely, so the new
// space starts with no user pages for exec() to fill.
static u64int clone_kernel_table(page_table_t *src, int level)
{
    u64int dst_phys = alloc_zeroed_table();
    page_table_t *dst = (page_table_t *)dst_phys;
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        {
            if (se->user)
                continue;                   // skip user leaves: exec supplies its own
            dst->entries[i] = *se;          // share the supervisor frame by reference
        }
        else
        {
            page_table_t *child = (page_table_t *)((u64int)se->frame << 12);
            u64int sub = clone_kernel_table(child, level - 1);
            dst->entries[i] = *se;
            dst->entries[i].frame = sub >> 12;
        }
    }
...
u64int new_kernel_address_space(u64int src_pml4_phys)
{
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        if (i >= 256)
        {
            // Upper canonical half -- the kernel. Shared by reference, exactly
            // as in clone_address_space.
            dst->entries[i] = src->entries[i];
        }
        else
        {
            // Lower half. Walk it, keeping supervisor pages and dropping user.
            page_table_t *pdpt = (page_table_t *)((u64int)src->entries[i].frame << 12);
            u64int sub = clone_kernel_table(pdpt, 3);
            dst->entries[i] = src->entries[i];
            dst->entries[i].frame = sub >> 12;
        }
    }
...
// The mirror image of clone_table, for teardown.
static void free_table(page_table_t *t, int level)
{
    for (int i = 0; i < 512; i++)
    {
        page_t *e = &t->entries[i];
        if (!e->present)
            continue;

        if (level == 1)
        {
            if (e->user)                       // a private user frame -- free it
                clear_frame((u64int)e->frame << 12);
            // supervisor frames are shared; leave them alone
        }
        else
        {
            free_table((page_table_t *)((u64int)e->frame << 12), level - 1);
        }
    }
    clear_frame((u64int)t);                     // free the table itself
}
...
void free_address_space(u64int pml4_phys)
{
    ...
    for (int i = 0; i < 256; i++)               // lower half only
        if (pml4->entries[i].present)
            free_table((page_table_t *)((u64int)pml4->entries[i].frame << 12), 3);
    clear_frame(pml4_phys);                      // the upper half is shared: leave it
}
