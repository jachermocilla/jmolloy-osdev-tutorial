...
// The ring 3 program, in user.c. It forks, and the child execs "prog".
...
int main(struct multiboot *mboot_ptr)
{
    ...
    // A ring 3 program that forks; the child execs the file "prog" from the
    // initrd and becomes it. The loaded program reads the keyboard, so it is a
    // first-class ring 3 process in every way -- and it arrived as a file, not
    // as a function compiled into this kernel.
    monitor_write("fork + exec: a ring 3 program forks; the child becomes a file.\n\n");
    int u = create_user_task(&thread_task);
    monitor_write("launched tid "); monitor_write_dec(u); monitor_write("\n\n");
