...
void initialise_tasking()
{
    ...
    current_task->pml4_phys  = current_pml4_phys;   // the kernel address space
    current_task->next       = 0;
...
int create_task(void (*entry)(void))
{
    ...
    task->pml4_phys  = current_pml4_phys;   // kernel threads share one space
...
registers_t *schedule(registers_t *regs)
{
    ...
    // The CR3 reload chapter 9 left a hole for. Load the incoming task's
    // address space. This is safe mid-interrupt precisely because the kernel
    // half -- including the stack we are standing on and the code we are about
    // to run -- is identical in every address space, shared by reference in
    // clone_address_space().
...
int create_user_task(void (*entry)(void))
{
    ...
    task->kstack     = kmalloc_a(KERNEL_STACK_SIZE);
    task->kstack_top = task->kstack + KERNEL_STACK_SIZE;
    task->pml4_phys  = current_pml4_phys;
...
int fork(registers_t *parent_frame)
{
    ...
    // Clone the caller's address space: kernel shared, user copied.
...
void task_exit(void)
{
    ...
    else
    {
        ...
        while (p->next && p->next != dying)
            p = p->next;
        if (p->next == dying)
            p->next = dying->next;
    }

    // Free the private address space -- but only if it is not the shared kernel
    // one. (Task 1 and kernel threads share kernel_pml4; never free that.)
    if (dying->pml4_phys != (u64int)kernel_pml4)
        free_address_space(dying->pml4_phys);

    // We cannot free our own kernel stack while standing on it, nor the task_t
    // we are reading. A real kernel hands these to a reaper; we leak them, and
    // say so. The address space -- the expensive part -- is reclaimed above.

    // Leave current_task pointing at us and dying->next pointing at our old
    // successor. The first yield saves our (now meaningless) state and advances
    // past us; because nothing in the ready queue points at us any more, the
    // round-robin can never select us again.
    asm volatile("sti");
    for (;;)
        task_yield();
}
