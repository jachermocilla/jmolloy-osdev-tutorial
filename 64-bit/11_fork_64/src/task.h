...
typedef struct task
{
    ...
    u64int pml4_phys;       // Physical address of this task's PML4 (its CR3).
...
// fork(). Clones the current address space and the current interrupt frame,
// producing a child that resumes from the same syscall with a return value of
// 0. Returns the child's pid to the parent. Must be called from within a
// syscall -- it needs the caller's interrupt frame.
int fork(registers_t *parent_frame);

// Removes the current task from the ready queue, frees its address space and
// kernel stack, and yields. Never returns.
void task_exit(void);
